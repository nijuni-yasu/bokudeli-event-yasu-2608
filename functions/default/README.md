...
### Storage トリガー（onObjectFinalized）の設計方針

`onObjectFinalized` は、**責務ごとに関数を分けて**定義してください。複数の処理を一つの関数にまとめてパスで分岐させる「catch-all」構造は避けてください。

**分ける理由:**

- 責務が明確になり、可読性・保守性が向上する
- メモリ・タイムアウト・リトライ等の設定を独立して行える
- ログ・モニタリング・デバッグが容易になる
- 新しいロジックを追加する際の副作用リスクが減る

**各関数が守るべきこと:**

- ファイルパスが自分の責務に合致しない場合は即座に `return` する
- 冪等性を保つ（同じファイルで複数回実行されても安全）
