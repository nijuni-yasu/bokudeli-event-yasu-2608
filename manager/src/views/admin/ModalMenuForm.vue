    ...
    methods: {
      validateForm (scope) {
        ...
        if (!me.menu_id) {
          ...
          storage
            .ref()
            .child('partners/' + me.$store.state.partner_id + '/menus/menu_' + fileName + '.jpeg')
            .put(this.menu_image_file).then(function (snapshot) {
              snapshot.ref.getDownloadURL().then(function (url) {
                // console.log(url) // ダウンロードURL取得
                // menu画像のアップロード完了後に、me.$store.state.partner_id直下にmenu情報のdoc追加
                // チェーン店などの系列ならメニュー情報はPartnerIdで共通。(将来的に)それを店舗ごとに表示/非表示する機能をいれる。
                db
                  .collection('partners')
                  .doc(me.$store.state.partner_id)
                  .collection('menus')
                  .add({
