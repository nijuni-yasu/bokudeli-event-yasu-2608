...
describe('shouldShowPfEventParticipantsSection', () => {
  it('参加者 0 人のとき PF / enterprise とも非表示', () => {
    expect(shouldShowPfEventParticipantsSection({ enterprise_id: 'ent-1' }, 0)).toBe(false)
    expect(shouldShowPfEventParticipantsSection({ enterprise_id: null }, 0)).toBe(false)
  ...
  it('enterprise イベントは 1 人以上ならしきい値に関係なく表示', () => {
    expect(shouldShowPfEventParticipantsSection({ enterprise_id: 'ent-1', members_visible_min_count: 10 }, 1)).toBe(
      true,
    )
    expect(shouldShowPfEventParticipantsSection({ enterprise_id: 'ent-1' }, 2)).toBe(true)
  })

  it('PF で members_visible_min_count 未設定は 1 人以上で表示', () => {
    expect(shouldShowPfEventParticipantsSection({ enterprise_id: null }, 1)).toBe(true)
