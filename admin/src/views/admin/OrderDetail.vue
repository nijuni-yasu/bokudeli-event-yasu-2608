  ...
  >
    ...
    >
      <div>
        ...
        <p>【URL】<a :href="getEventUrl(event.community_account, event.event_id)">{{ getEventUrl(event.community_account, event.event_id) }}</a></p>
      </div>
      ...
      <div v-else-if="event.event_status == 'accepting_order'">
        ...
        <v-simple-table>
          ...
          <tbody>
            <tr>
              <td>{{ ordersTotal.reduce((sum, o) => sum + o.count, 0) }}</td>
              <td>{{ ordersTotal.reduce((sum, o) => sum + o.count * o.menu_price, 0) }}</td>
            </tr>
          </tbody>
        </v-simple-table>
      </div>
