---
name: git-fixup
description: 変更を過去の適切なコミットに fixup + autosquash で統合する。コミットメッセージはそのまま。「fixupして」「フィックスアップして」「過去のコミットにまとめて」と依頼された時に使用する。吸収先はいま作業中のブランチ上のコミットに限定する。すでに origin/development にマージ済みの古いコミットを吸収先にすると rebase 範囲が広がりリモートと乖離しやすい。
---
...
- すでに **`origin/development` に取り込まれた履歴のコミット** を吸収先にすると、`git rebase … <吸収先>~1` が共通往昔まで遡り、**全履歴の書き換え・リモートとの大幅乖離**につながる。ユーザーが明示しない限り、そのような吸収先は **選ばない**。迷う場合はユーザーに確認する。
...
**禁止**: `origin/development` にすでにマージ済みの過去コミットだけを吸収先にする（rebase 起点が共通往昔まで広がる）。吸収先は **いまのブランチに載っているコミット** に絞る。
