...
description: ステージングされている変更差分を読み込んでコミットメッセージを生成する。コミット前にメッセージを検討したい時、コミットメッセージを考えてほしいと依頼された時、または git-commit-workflow / git-fixup / git-squash / git-split-commit から委譲されたメッセージ生成に使用する。メッセージフォーマットの正本は本スキルのみ。
---
...
1. 呼び出し元コンテキスト（下記）を確認する
2. `git diff --cached --stat` でステージング差分のサマリーを取得する
3. `git diff --cached` で全差分を取得する
4. `git branch --show-current` でブランチ名を取得し Issue 番号を特定する
5. 下記フォーマットに従ってコミットメッセージを生成する

## 呼び出し元コンテキスト

呼び出し元から追加情報がある場合は、ステージング diff とあわせて解釈する。フォーマット正本は本スキルのみ。他スキルにフォーマット節を書かない。

| コンテキスト                                        | 入力                             | 生成方針                                                                                                   |
| --------------------------------------------------- | -------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| **通常**（C 新規1件、分割各コミット、ユーザー依頼） | ステージング diff のみ           | diff に基づく新規メッセージ                                                                                |
| **squash 書き換え**（A2）                           | 元メッセージ全文 + 統合後 diff   | 元メッセージと diff を踏まえ、**統合後のコミット全体**を正確に説明する新メッセージ。タイトルも必要なら更新 |
| **amend**（A0）                                     | 現 HEAD メッセージ + 追加分 diff | 統合後の HEAD を説明するメッセージ。乖離がなければ呼び出し元が `--no-edit` を選ぶ                          |
