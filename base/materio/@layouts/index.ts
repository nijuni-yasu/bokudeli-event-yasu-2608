...
import { cookieRef, useLayoutConfigStore } from './stores/config'
...
export const createLayouts = (userConfig: PartialDeep<LayoutConfig>): Plugin => {
  return (): void => {
    ...
    layoutConfig.app.iconRenderer =
      (userConfig.app?.iconRenderer as LayoutConfig['app']['iconRenderer']) ?? layoutConfig.app.iconRenderer

    layoutConfig.verticalNav.defaultNavItemIconProps =
      userConfig.verticalNav?.defaultNavItemIconProps ?? layoutConfig.verticalNav.defaultNavItemIconProps
    ...
    configStore.$patch({
      ...
      appContentWidth: cookieRef('appContentWidth', userConfig.app?.contentWidth ?? layoutConfig.app.contentWidth)
        .value,
      ...
      isVerticalNavCollapsed: cookieRef(
        ...
        userConfig.verticalNav?.isVerticalNavCollapsed ?? layoutConfig.verticalNav.isVerticalNavCollapsed,
      ).value,
