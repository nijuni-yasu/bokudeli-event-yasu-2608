...
**カード右下（全キャンセル時）のラベル決定**: 注文ごとのラベルが 1 種類ならそれを表示する。混在する場合は次の優先順位で 1 つに丸める。

| 順位 | 条件 | 表示 |
|:--|:--|:--|
| 1 | 1 件でも `canceled_event` がある | **イベント中止（返金済み）** |
| 2 | それ以外（`user` と `organizer_reject` の混在等） | **キャンセル済み** |

イベント中止は注文単位ではなくイベント単位の事実であるため、混在時は `canceled_event` を優先する。
