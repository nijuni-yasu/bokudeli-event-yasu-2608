...
| Phase 2: データ移行 | 旧 orders → 新 members + member_orders + stripes にバッチコピー。`in_cart` はスキップ。旧データは変更しない | 自動 | 低 |
...
- ドライラン（`MIGRATE_ORDERS_DRY_RUN`）では **書き込みは行わないが、既存 `member_orders` の読み取り（get）は行う**。
