...
| 0-3-2b | Rules CI 必須チェック（WS-A / A-5） | `firestore.rules` または `tests/firestore-rules/**` を変更する PR では **`Test Firestore Rules / test`** も green 必須。branch protection の required checks に追加する（Rules 非変更 PR では workflow が skip されるため、GitHub 設定では「Require status checks to pass」に **skipped を許容**するか、Rules 変更 PR 時のみ手動確認する） | - | Todo |
