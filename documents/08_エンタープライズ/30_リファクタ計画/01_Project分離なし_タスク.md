...
- **2026-06-27**: **C-1 PoC（MVP）** — [05_WS-C_C-1_PoC設計](./05_WS-C_C-1_PoC設計.md)（H1 採用案・G1）。PA-10a〜11d 本実装は Phase 2（G-7）。
...
- **v0.3 のスキーマ確定** — [ADR-003](../20_設計判断_ADR/ADR-003_publish_scope移行.md) + [10_仕様/04_詳細_イベント管理](../10_仕様/04_詳細_イベント管理.md) で `community.join_type` / `event.publish_scope` 等が確定。**G2 ✅（仕様のみ）**。PA-11 本実装は Phase 2。
...
| **やる（条件付き・Phase 1）** | IdP **認証モデル**（§1 (a)・PA-01〜03・テナント onboarding/メアド検索のテナント化） | IdP 認証モデルは**移行コストの非対称性**から本番データ前に前倒し（`07` 確定方針）。スキーマ型分岐・3 軸モデルは **Phase 2**（2026-06-27） |
| **やる（Phase 2）** | スキーマ型分岐（§2 PA-10/11）／3 軸・publish_scope Migrate／functions 論理ディレクトリ整理（§3 PA-20b-c） | PF 版と合わせて横断設計。PA-11e ✅ は MVP で完了 |
...
| **Phase 1（v0.2 内で前倒し・移行に敏感）** | **IdP 認証モデル**: PA-01a〜e／PA-02a〜f／PA-03a〜c。**C-1 PoC + G1**: PA-10a（[05_WS-C_C-1](./05_WS-C_C-1_PoC設計.md)） | **IdP 認証モデルは本番データ前に**（移行コストの非対称性・`07` 確定方針）。C-1 は MVP 中に方式確定のみ（本番 schema 変更なし） |
| **Phase 2（MVP 後・PF 横断）** | スキーマ分岐 PA-11a〜e（G-7）／3 軸・publish_scope Migrate（G-5〜G-6）／ゲスト/SSO/UI・ディレクトリ再編 | PF 版参加仕様と合わせて実施（[01_MVP全体計画](../00_計画/01_MVP全体計画.md) 2026-06-27） |
...
> **フェーズ（§5.1）**: **2026-06-27 更新**。PA-10a（C-1 PoC + G1）は **MVP 中** — 正本 [05_WS-C_C-1_PoC設計](./05_WS-C_C-1_PoC設計.md)。PA-11a〜d 本実装・3 軸・publish_scope Migrate は **Phase 2**（WS-G G-5〜G-7）。**PA-11e partner 互換テストは ✅**（現行 optional 拡張向け・C-5）。

| 子ID | タスク | 対象 | 完了条件 | 依存 | 難易度 |
|:--|:--|:--|:--|:--|:--|
| PA-10a | 3 方式の比較検証（discriminatedUnion / extend / ネスト）を PoC。**PR-C1b** で `eventWrite.ts` を package 昇格 | `common/src/schemas/poc/`・`common/src/schemas/eventWrite.ts`（[05_WS-C_C-1](./05_WS-C_C-1_PoC設計.md)） | H1 採用 + G1 通過 | — | 中 |
...
| PA-11e | partner が base 形で parse できる互換テスト追加 ✅（MVP / C-5） | `common` テスト | 混在データで parse 成功 | —（現行 optional 方式） | 中 |
...
7. **MVP 中: PA-10a（C-1 PoC）→ G1**（[05_WS-C_C-1_PoC設計](./05_WS-C_C-1_PoC設計.md)。WS-B/D/F と並行可）

**Phase 2（後続・移行に鈍感）**

8. **PA-11a-e スキーマ分岐本実装 + 3 軸・publish_scope Migrate**（PF 版と合わせて — [01_MVP全体計画](../00_計画/01_MVP全体計画.md) G-5〜G-7）
9. **IdP のゲストフロー（デフォルトプール固定・`07` §3）／企業別 SSO（`07` §1.5）／オープン交流の扉（`07` §5.1）**
10. **PA-20b-c functions ディレクトリ再編 ＋ PA-24c マルチ codebase 化（必要時）**
...
| 2026-06-27 | 3 軸モデル・publish_scope Migrate・PA-10/11 本実装を Phase 2 へ移行。MVP は `is_public` ＋ PA-11e ✅ |
