...
- **関連 Issue**: [#2214](https://github.com/nijuniinc/bokudeli-event-new/issues/2214) 〜 [#2220](https://github.com/nijuniinc/bokudeli-event-new/issues/2220)、[#2227](https://github.com/nijuniinc/bokudeli-event-new/issues/2227)（ラベル `チャット機能`）
...
| V2-8 | 未送信 compose 保持（セッション） | 改善 | M | 不要 | 中 |

**v2 スコープ外（確定）**: チャット新着のメール通知 — レター（#21）および FCM（[`02_プッシュ通知.md`](./02_プッシュ通知.md)）に役割分担。

**V2-8 補足**: 本文・選択済み添付（`File`）は Pinia（[`chatComposeDraft`](../../base/src/stores/chatComposeDraft.ts)）にルーム単位で保持。**同一タブ・ページリロード前**のみ復元。ルーム切替・チャット画面 Unmount 時に store へ保存する。**同一ブラウザでログインアカウント（Firebase UID）が変わった場合**は、プライバシー混在防止のため Pinia 上の下書きをすべて破棄し、入力欄もクリアする（`ChatApp` の `currentUserId` watch）。リロード後の永続化（IndexedDB 等）は Phase 2・別 Issue 検討。
...
- 拡大表示は [`AlbumLightbox.vue`](../../base/src/components/AlbumLightbox.vue)（`vue-easy-lightbox`）。ライトボックス表示中は Teleport で **表示中スライド 1 枚** の DL ボタンを表示
...
| D-2a | メッセージログのアクション列（絵文字・3点リーダーの隣）から、メッセージ内の**全添付画像**を一括ダウンロードできる |
| D-2b | 拡大表示（ライトボックス）から、**表示中の 1 枚**をダウンロードできる |
...
| 場所 | 変更ファイル | 内容 |
|------|-------------|------|
| メッセージ一覧 | `ChatMessageReactions.vue` + `ChatLog.vue` | 絵文字・3点リーダーの隣に一括 DL ボタン。添付 1 枚以上で表示 |
| 拡大表示 | `ChatLog.vue` | ライトボックス表示中、右下に DL ボタン（Teleport）。**表示中スライド 1 枚**の `title` を fileName に |

#### 3. i18n

`base/src/locales/messages/ja.ts` に追加:

- `chat.download_attachment`: 「画像をダウンロード」（ライトボックス用）
- `chat.download_all_attachments`: 「画像をすべてダウンロード」（メッセージ一覧の一括 DL 用 `aria-label`）
- `chat.download_failed`: 「画像のダウンロードに失敗しました」
...
| `functions/default/src/index.ts` | export 追加時は import / export を更新（deploy yml 手書き不要） |
...
| R-2 | 絵文字は **固定 6 種**: 👍 ❤️ 😆 😭 🎉 🙏（将来 `+` で全 emoji 選択は別 Issue） |
| R-3 | 1 ユーザー 1 メッセージあたり **1 リアクション**（同 emoji で toggle 削除、別 emoji で上書き） |
| R-4 | リアクション追加時の **プッシュ通知は v2 では送らない** |
| R-5 | 送信取り消し時: リアクションも非表示（message 論理削除のため UI 非表示） |
| R-6 | ended / readonly ルームではリアクション **追加・変更・削除禁止**（既存サマリは閲覧のみ） |
| R-7 | V2-6 既読 UI とは独立（既読実装は凍結） |
| R-8 | 😊 ピッカー操作直後に **楽観更新**（Firestore 応答前にサマリ表示を更新）。失敗時ロールバック。ページング外メッセージも対象 |
| R-9 | サマリは **1 つの pill**、白背景、`CHAT_REACTION_EMOJIS` 順で emoji **連続表示**（例: `❤️❤️😆`）。数字は出さない |
| R-10 | サマリ tap はリアクション操作しない。追加・変更・削除は **😊 ピッカーのみ** |
| R-11 | サマリ tap で **リアクション詳細ダイアログ**。`reactions` を open 時 getDocs、`created_at` 昇順、各行 = アバター + 表示名 + emoji |

### データモデル

```
chat_rooms/{roomId}/messages/{messageId}/reactions/{userId}
  emoji: string          // CHAT_REACTION_EMOJIS 内
  created_at: Timestamp
  updated_at: Timestamp
```

message doc への denormalize（**#2220 同 PR で必須**）:

```typescript
reaction_summary: { '👍': 3, '❤️': 1 }  // onChatReactionWritten Trigger で更新
```

クライアントは reactions サブコレクションのみ read/write。`message.reaction_summary` は Admin SDK のみ更新（messages Rules `update: false` 維持）。

### Rules

- read: ルーム member
- create/update/delete: `isChatRoomWritable(roomId)` かつ自分の `{userId}` のみ、emoji は許可 6 種

### UI

- **横側（バブル横）**: 時刻、`⋮` 送信取り消し（自分のみ）、添付一括 DL。PC は hover / focus-within で ⋮・DL を表示し、非 hover 時は時刻のみ。タッチ端末は ⋮・DL（上）と時刻（下）を常時縦並び
- **下側（バブル下）**: `mdi-emoticon-happy-outline`（😊 ピッカー）を summary pill の**左**に配置。リアクション未付与時は 😊 のみ表示
- 😊 クリック → 横並びピルに 6 emoji。選択で `toggleReactionWithOptimistic`（R-8）
- バブル下に **1 つの統合 pill**（白背景、`reaction_summary` を emoji 連続表示。R-9）。tap で詳細ダイアログ（R-11）。リアクション操作は行わない（R-10）
- ended / readonly ルーム: ピッカー非表示、サマリ + 詳細は閲覧可
- 変更: [`ChatMessageReactions.vue`](../../base/src/components/chat/ChatMessageReactions.vue)、[`ChatReactionDetailDialog.vue`](../../base/src/components/chat/ChatReactionDetailDialog.vue)（新規）、[`ChatLog.vue`](../../base/src/components/chat/ChatLog.vue)

### store / Functions

- [`common/src/utils/chatReactionSummary.ts`](../../common/src/utils/chatReactionSummary.ts): `applyOptimisticReactionSummary`, `formatReactionSummaryText`
- [`base/src/stores/chat.ts`](../../base/src/stores/chat.ts): `patchMessageReactionSummary`, `myReactionByMessageId`
- [`base/src/stores/chatReaction.ts`](../../base/src/stores/chatReaction.ts): `toggleReaction`, `toggleReactionWithOptimistic`, `listChatReactions`
- [`functions/default/src/onChatReactionWritten.ts`](../../functions/default/src/onChatReactionWritten.ts): `reaction_summary` denormalize

#### 工数

- 確定見積: **M**（denormalize Trigger 含む）

### GitHub Issue

- 実装: [#2220](https://github.com/nijuniinc/bokudeli-event-new/issues/2220)（PR タイトルは `[common][firebase][functions][base]` 推奨）
...
| V2-8 | [#2227](https://github.com/nijuniinc/bokudeli-event-new/issues/2227) | `[base][user] チャット未送信入力（本文・添付）をセッション内で保持する` | — |
...
| 2026-07-23 | V2-7 要件 R-8〜R-11 追加: 楽観更新・統合 pill 表示・詳細ダイアログ |
| 2026-07-31 | V2-8 未送信 compose 保持（#2227）。Phase 1 は Pinia セッション内のみ |
