...
### UI（Facebook Messenger 同等）

- メッセージ横に `mdi-emoticon-happy-outline`（`⋮` 送信取り消しメニューの隣。自分メッセージのみ `⋮` も表示）
- PC: メッセージ hover / focus-within でアクション行表示。タッチ端末: 常時表示
- 😊 クリック → 横並びピルに 6 emoji。選択で `toggleReaction`
- バブル下に `reaction_summary` チップ（`👍 3`）。writable 時タップで toggle、readonly 時は閲覧のみ
- 変更: [`ChatMessageReactions.vue`](../../base/src/components/chat/ChatMessageReactions.vue)（新規）、[`ChatLog.vue`](../../base/src/components/chat/ChatLog.vue)

### store / Functions

- [`base/src/stores/chatReaction.ts`](../../base/src/stores/chatReaction.ts): `toggleReaction`
