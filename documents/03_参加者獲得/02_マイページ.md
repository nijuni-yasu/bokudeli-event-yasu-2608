...
| `friend_count` | number (int, ≥0) | **required**（同上） | **確定**: **`getUserFriends`（友人 Callable）と同一の除外ルール**に合わせたともだち数。`users/{uid}/friends` の生ドキュメント数ではなく、**退会ユーザー（`is_deleted == true`）等はカウントに含めない** |
...
- 退会済みユーザー本人の `users` ドキュメントの **カウントは凍結**（再集計しない）。**他ユーザーの `friend_count` / `joined_community_count` / `managed_community_count`** は、退会者を **数えない**（`getUserFriends` の除外ルールと一致させるため、関連する **両側の users** の再集計が必要）
- **未実装（別イシュー）**: アカウント削除フローから退会者に紐づく友人・コミュニティ側の `recountUserProfileCounts` を一括起動する処理は Phase 1 では未接続（RC-50）。表示上は `getUserFriends` / Callable の退会者除外で整合する
...
- [ ] 退会時の他者 `friend_count` 再集計（§5.1.2・RC-50・別イシュー）
