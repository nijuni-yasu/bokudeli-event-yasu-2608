...
- **（任意）** [04_プロフィールタグ機能.md](./04_プロフィールタグ機能.md): プロフィールタグ・イベントタグ（Phase 1 同時リリース）との整合
...
| --- | --- | --- | --- |
| プロフィール所有者本人 | する | **する** | **する**（`EventCard` / プロフィールイベントプレビュー。`private_event`） |
| 本人以外（他ユーザー・未ログイン） | **する** | **しない** | **する**（リンク不可でもチップで公開範囲を明示） |

- **一般公開**（`is_public == true`）: 本人・他者とも **表示しリンクする**（限定公開チップは出さない）
...
- **例外**: 注文履歴タブの `UserEventCard` は操作 UI のため **URL限定公開チップを出さない**（`hidePrivateScopeChip`）。meet log ダイアログ内の各行にもチップは出さない
- 他者に見えるのは **参加実績のメタ情報**（名称・サムネ・日時等）。詳細 URL の直叩きは **既存の Firestore ルール・ルーティング**で拒否する（§5.4）

#### 4.2.1 プロフィールタブ

- **表示**: 画像・表示名・自己紹介・SNS リンク（既存のプロフィール情報）
- **ローディング（確定・2026-06-19）**: `getUserProfilePreview` 初回取得中も **数値サマリー・各プレビューの v-card 枠は先に表示**し、各ブロック内を **スケルトン / プレースホルダー**でローディングする（タブ全体をスピナーのみにしない）
...
- **限定公開イベント（確定・2026-06-02 / チップ追加 2026-06-19）**: §4.2.0。本人・他者ともカード・サムネ・タイトルを表示。**本人のみ**イベント詳細へリンク。`EventCard` に **`URL限定公開` チップ**を表示
...
- **キャンセル済み注文（確定）**: 全キャンセル後もイベントカードを表示し、ステータスラベルを出す（`member_orders` 起点の一覧。`Event.members` から外れても表示）
- **ステータスラベル（`cancel_source` 別）** — 正本: `09_EventMemberOrderに伴うキャンセル機能.md` §「キャンセルメタデータ」
  - 参加者自身のキャンセル: **キャンセル済み**
  - イベント都合（最小催行自動中止・主催者一括中止等）: **イベント中止（返金済み）**
  - 将来・主催者による個別拒否: **参加取消（返金済み）**
  - 既存データ（`cancel_source` 未設定）: イベントが `event_canceled` ならイベント中止系、それ以外はキャンセル済みと推定表示
...
- **限定公開タグ**: イベントカード（`EventCard` / プロフィールイベントプレビュー）では **`private_event` チップを表示**。注文履歴の `UserEventCard` と meet log ダイアログは **出さない**
