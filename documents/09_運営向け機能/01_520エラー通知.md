...
`user` / `partner` アプリで予期しないクライアントエラー（Zod バリデーション失敗、Vue レンダリングエラー、未捕捉 Promise 拒否など）が発生した際、ユーザーには既存の 520 エラーページを表示しつつ、運営側が **Cloud Logging で状況を追跡・アラート通知** できる仕組みを構築する。
...
| 区分 | 説明 |
|:--|:--|
| **エラー発生者** | `user` / `partner` アプリの一般ユーザー・主催者・店舗管理者（520 ページが表示される） |
...
- ルート: `/520`（`user/src/pages/[[...error]].vue` / `partner/src/pages/[[...error]].vue`）
- 遷移元: `user/src/main.ts` / `partner/src/main.ts` が mount 後に呼び出す `setupGlobalErrorHandling`（`base/src/utils/setupGlobalErrorHandling.ts`）内の `app.config.errorHandler`
...
- `user` / `partner` のグローバルエラーハンドラ（`main.ts` → `setupGlobalErrorHandling`）
...
**呼び出し**: `user/src/main.ts` / `partner/src/main.ts` が `app.mount('#app')` 後に dynamic import で実行
...
```typescript
// 処理概要
// 1. configureClientErrorReporting({ app }) で user/partner を起動時注入（store 経由レポート用）
...
`user/src/main.ts` / `partner/src/main.ts` は `Promise.all` で plugin 読み込み後、**mount 前**に `configureClientErrorReporting({ app: 'user' | 'partner' })` を呼び出す。`setupGlobalErrorHandling` は mount 後に dynamic import で実行する。

```typescript
// user/src/main.ts, partner/src/main.ts（概要）
import { configureClientErrorReporting } from '@shokujii/base/utils/reportClientError.js'

Promise.all([/* plugins */]).then((plugins) => {
  configureClientErrorReporting({ app: 'user' }) // partner は 'partner'
...
6. dedup 通過後、`isClientErrorNoise` で判定し Cloud Logging に記録
   ...
   - **ノイズ**（chunk / SW / Rejected 等）→ `.warn(...)`（`jsonPayload.severity=warn`）。Monitoring アラート対象外
7. 統合 Log-based Alert `shokujii functions error` が ERROR を検知した場合のみ運営に通知（[§9.3](./02_Monitoringアラート_shokujii_functions_error.md) 参照）

#### 5.8.2 store 内サイレント失敗時（store 層）

1. `onSnapshot` コールバック内で `doc.data()` が ZodError を throw
2. `catch` で `reportClientError(..., { severity: 'warn' })` を呼ぶ（**520 には遷移しない**。ユーザー体験を止めない store 層の意図）
3. Callable `reportClientError` が payload を検証し、`isClientErrorNoise` で server 側 severity を決定
   - **ZodError**（`error_type=ZodError`）→ 常に `logger.error`（`jsonPayload.severity=error`）。client が `severity: 'warn'` を送っても server 正本が優先され、`client_severity: warn` が併記される（[#2166](https://github.com/nijuniinc/bokudeli-event-new/issues/2166)）
   - **ノイズ**（chunk / SW 等）→ `logger.warn`（Monitoring アラート対象外）
   - **その他の要対応** → `logger.error`
4. 統合 Log-based Alert `shokujii functions error` は **Cloud Logging の ERROR** を検知するため、store 由来の ZodError もアラート対象（[§9.3](./02_Monitoringアラート_shokujii_functions_error.md) 参照）。運用調査時は `client_severity=warn` と `severity=error` の両方を参照する
...
- [x] `user/src/main.ts` / `partner/src/main.ts` から `setupGlobalErrorHandling` 呼び出し
- [x] 主要 store の catch への hook（`event.ts`, `eventList.ts`, `community.ts`, `communityList.ts`, `partner.ts`, `letter.ts`。`subscribeOrders` 含む）
- [x] 開発環境での送信スキップ

### 6.2 Phase 1（運用・デプロイ）

- [x] Functions `reportClientError` のデプロイ（初回）
- [x] `user` / `partner` アプリのデプロイ（初回）
- [x] GCP Log-based Alert 設定（[#2166](https://github.com/nijuniinc/bokudeli-event-new/issues/2166)。正本: [02_Monitoringアラート_shokujii_functions_error.md](./02_Monitoringアラート_shokujii_functions_error.md)）
- [ ] `#2166` clientError ノイズ WARN 格下げの Functions 再デプロイ（`reportClientError`）
...
- **フロントエンド**: `user` / `partner` の `main.ts`、`base/src/utils/setupGlobalErrorHandling.ts`、一部 `base` store
...
| ファイル | 現状 | 本機能での扱い |
|:--|:--|:--|
| `user/src/main.ts` / `partner/src/main.ts` | mount 後に `setupGlobalErrorHandling` を呼び出し | グローバル errorHandler + unhandledrejection を有効化 |
...
3. `user` / `partner` アプリをデプロイ済みであること（`setupGlobalErrorHandling` が有効化される）
...
```
jsonPayload.module="clientError"
jsonPayload.app="partner"
```

> リネーム前のログは `jsonPayload.app="admin"` のまま残る。調査時は必要に応じて OR 条件で含める。
>
> partner リネーム（PR #2057）マージ時のデプロイ順序・ClientError 移行の注意: [20_adminからpartnerへリネーム](../07_リファクタリング/20_adminからpartnerへリネーム.md) の「マージ前チェックリスト」「マージ後のデプロイ順序と注意」を参照。

### 9.3 Log-based Alert 設定

本番では **統合アラート** `shokujii functions error` を使用する（clientError 専用ポリシーではない）。filter 正本・変更手順は [02_Monitoringアラート_shokujii_functions_error.md](./02_Monitoringアラート_shokujii_functions_error.md) を参照。

| 項目 | 設定値 |
|:--|:--|
| 条件 | Log match condition |
| クエリ | `severity>=ERROR` かつ clientError **ノイズのみ除外**（ZodError は通知対象） |
| 通知先 | 運営メール（Monitoring 通知チャネル） |

**server 側の二重防御**（`#2166`）: [`clientErrorReport.ts`](../../functions/default/src/clientErrorReport.ts) が [`clientErrorNoise.ts`](../../functions/default/src/utils/clientErrorNoise.ts) でノイズを `logger.warn` に格下げ。ZodError は常に `logger.error`。

warn レベル（store 内サイレント失敗・clientError ノイズ）はアラート対象外。調査時に Logs Explorer で `severity>=WARNING` を参照する。
